...
class WordPressMLMAccessControl 
{
	function admin_init()
	{
		...
		if ( ! load_plugin_textdomain( 'mlm', plugin_dir_url( __FILE__ ) . '/languages/' ) ) {
			load_plugin_textdomain( 'mlm', plugin_dir_url( __FILE__ ) . '/wordpress-access-control/languages/', 'wordpress-access-control/languages/' );
		}
	}
		...
		if ( isset( $_REQUEST['action'] ) && $_REQUEST['action'] == 'update' ) {
			if ( ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'mlm_options_save' ) ) {
				...
				if ( isset( $_REQUEST['mlm_custom_post_types'] ) && is_array( $_REQUEST['mlm_custom_post_types'] ) ) {
					update_option( 'mlm_custom_post_types', filter_var($_REQUEST['mlm_custom_post_types'], FILTER_SANITIZE_STRING) );
				} else {
				...
				if ( isset( $_REQUEST['page_excerpt_text'] ) ) {
					update_option( 'mlm_page_excerpt_text',  filter_var($_REQUEST['page_excerpt_text'], FILTER_SANITIZE_STRING) );
				}
				
				if ( isset( $_REQUEST['post_excerpt_text'] ) ) {
					update_option( 'mlm_post_excerpt_text',  filter_var($_REQUEST['post_excerpt_text'], FILTER_SANITIZE_STRING) );
				}
