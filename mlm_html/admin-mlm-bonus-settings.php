...
$_POST  = filter_input_array(INPUT_POST, FILTER_SANITIZE_STRING);
		...
		$pair =  sanitize_text_field( $_POST['pair']);
	...
	<table border="0" cellpadding="0" cellspacing="0" width="100%" class="form-table">
		<tr>
			...
			<td>
				...
				<option value="pair" <?php echo  sanitize_text_field( $_POST['bonus_criteria'])=='pair' ? 'selected':''?>>No. of Pairs</option>
				<option value="personal" <?php echo  sanitize_text_field( $_POST['bonus_criteria'])=='personal' ? 'selected':''?>>No. of Personal Referrer</option>
				</select>
	...
	<table border="0" cellpadding="0" cellspacing="0" width="100%" class="form-table">
		<tr>
			...
			<td>
				<select name="bonus_criteria" id="bonus_criteria">
				<option value="">Select Bonus Criteria</option>
				<option value="pair" <?php echo  $mlm_settings['bonus_criteria']=='pair' ? 'selected':''?>>No. of Pairs</option>
			<option value="personal" <?php echo  $mlm_settings['bonus_criteria']=='personal' ? 'selected':''?>>No. of Personal Referrer</option>
				</select>
				<div class="toggle-visibility" id="admin-mlm-bonus"><?php _e('Please select bonus type.')?></div>
			</td>
		</tr>
		...
		?>
        	<TR>
				...
				<TD> <INPUT type="text" name="pair[]" size="15" value="<?php echo  $mlm_settings['pair'][$i]?>"/> </TD>
				<TD> <INPUT type="text" name="amount[]" size="15" value="<?php echo  $mlm_settings['amount'][$i]?>"/> </TD>
        	</TR>    	
		<?php
				$i++;
			}
