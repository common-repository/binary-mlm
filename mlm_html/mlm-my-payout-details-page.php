	...
	if(count($detailArr)>0)
	{
		...
		<table width="100%" border="0" cellspacing="10" cellpadding="1" class="payout-summary">
			...
			<tr>
				...
				<td width="50%" class="right"><?php echo $mlm_settings['currency'].' '.$detailArr['subtotal'] ?></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td width="50%" class="right"><strong><?php echo $mlm_settings['currency'].' '.$detailArr['netamount'] ?></strong></td>
			</tr>
...
function my_payout_details_function()
{
	if ( is_user_logged_in() && isset($_REQUEST['pid']))
	{
		...
		$sql = "SELECT {$table_prefix}mlm_users.id AS id , {$table_prefix}mlm_users.user_key FROM {$table_prefix}users,{$table_prefix}mlm_users WHERE {$table_prefix}mlm_users.username = binary {$table_prefix}users.user_login AND {$table_prefix}users.ID = '".$userId."'"; 
		...
		$sql = 	"SELECT 
				...
				WHERE 
					payout_id = '".filter_var($_REQUEST['pid'], FILTER_SANITIZE_STRING)."' AND 
					user_id = '".$mlm_user_id."'";
		 ...
		 if(!empty($rs))
		 {
			...
			$payoutDetail['payoutId'] = esc_attr($_REQUEST['pid']); 
