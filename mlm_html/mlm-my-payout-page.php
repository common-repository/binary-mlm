...
$mlm_settings = get_option('wp_mlm_general_settings');
	...
	<table width="100%" border="0" cellspacing="10" cellpadding="1" id="payout-page">
		...
		<tr>
			...
			<td><a href="<?php bloginfo('url')?>/?page_id=<?php echo  $page_id?>&pid=<?php echo  $row->payout_id?>">View</a></td>
...
function my_payout_function()
{
  ...
  $userId = $current_user->ID; 
	...
	//$mlm_user_id = $wpdb->get_results( );
	 ...
	 $sql = "SELECT {$table_prefix}mlm_users.id AS id FROM {$table_prefix}users,{$table_prefix}mlm_users WHERE {$table_prefix}mlm_users.username = binary {$table_prefix}users.user_login AND {$table_prefix}users.ID = '".$userId."'"; 
